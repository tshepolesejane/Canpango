...
describe('Tutorial part 6', () => {
  ...
  function getPageElts() {
    ...
    return {
      ...
      appHeroes: element(by.css('app-root app-beers')),
      allHeroes: element.all(by.css('app-root app-beers li')),
      selectedHeroSubview: element(by.css('app-root app-beers > div:last-child')),

      heroDetail: element(by.css('app-root app-beer-detail > div')),
  ...
  describe('Heroes tests', () => {
    ...
    it('can switch to Heroes view', () => {
      ...
      expect(page.allHeroes.count()).toEqual(10, 'number of beers');
    });
    ...
    it(`deletes ${newHeroName} from Heroes list`, async () => {
      const beersBefore = await toHeroArray(getPageElts().allHeroes);
      const li = getHeroLiEltById(targetHero.id);
      li.element(by.buttonText('x')).click();

      const page = getPageElts();
      expect(page.appHeroes.isPresent()).toBeTruthy();
      expect(page.allHeroes.count()).toEqual(9, 'number of beers');
      const beersAfter = await toHeroArray(page.allHeroes);
      // console.log(await Hero.fromLi(page.allHeroes[0]));
      const expectedHeroes =  beersBefore.filter(h => h.name !== newHeroName);
      expect(beersAfter).toEqual(expectedHeroes);
    ...
    it(`adds back ${targetHero.name}`, async () => {
      ...
      const beersBefore = await toHeroArray(getPageElts().allHeroes);
      const numHeroes = beersBefore.length;

      element(by.css('input')).sendKeys(newHeroName);
      element(by.buttonText('add')).click();

      let page = getPageElts();
      let beersAfter = await toHeroArray(page.allHeroes);
      expect(beersAfter.length).toEqual(numHeroes + 1, 'number of beers');

      expect(beersAfter.slice(0, numHeroes)).toEqual(beersBefore, 'Old beers are still there');

      const maxId = beersBefore[beersBefore.length - 1].id;
      expect(beersAfter[numHeroes]).toEqual({id: maxId + 1, name: newHeroName});
    });
    ...
    it('displays correctly styled buttons', async () => {
      element.all(by.buttonText('x')).then(buttons => {
        for (const button of buttons) {
          ...
          // Styles defined in beers.component.css
